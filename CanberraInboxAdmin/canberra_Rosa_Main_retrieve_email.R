...
#################################GMAIL TO COMMUNICATE THE RETRIEVED WAS DONE##############

#Create a code to Email Daniel that the retrieved job is done and request do the manual job should be run
# Create the MIME message
# email <- gm_mime() %>%
#   gm_to("canberra.inbox@gmail.com") %>%
#   gm_from("canberra.inbox@gmail.com") %>%
#   gm_subject("CBR_Inbox: Request Manual Check PHID") %>%
#   #gm_text_body("Hi Daniel, CBR_Inbox_TEM.csv has been creaded. Please Check the PHID in the datset.
#               #  Add Yes NO in Ground_Truth column.
#                # Where there is a incorrect match, you need to change only the PHID
#                #(remember that similarity and Name columns were added just to help you do the comparison).
#               # Then save the file with the name CBR_Inbox_TEM_approved.csv")
#   gm_text_body("Hi Daniel,
# 
# The file CBR_Inbox_TEM.csv has been created. Please check the PHID column in the dataset.
# 
# In the Ground_Truth column, please add \"Yes\" or \"No\". If there is an incorrect match, you only need to change the PHID (the similarity and Name columns are included to help you with the comparison).
# 
# Once done, save the file with the name CBR_Inbox_TEM_approved.csv.
# 
# Thank you!")
# 
# # Send the email
